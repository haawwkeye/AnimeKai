	...
	try {
		...
		if (dub.length > 0) {
			try {
				...
				console.error("Dub Stream exists but URL doesn't details:");
				showError(error);
			}
		}
		if (sub.length > 0) {
			try {
				...
				console.error("Hard Sub Stream exists but URL doesn't details:");
				showError(error);
			}
		}
		if (softsub.length > 0) {
			try {
				const streamUrl = await GetStreamURL(softsub);
				if (streamUrl) {
					streams.push("SOFT SUB");
					streams.push(streamUrl);
				}
			} catch (error) {
				console.error("Soft Sub Stream exists but URL doesn't details:");
				showError(error);
			}
		}
